...
var TemplateManageJS = {
    ...
    queryTemplateByCondition : function(pageNum){
        ...
        if(condition=='请输入模板名称'&&begindate==''&&enddate=='') {
            window.location.href = path + "/templateManage/showTemplateManage?pageNum=1";
            return;
        }
        ...
        $.ajax({
            url:path+"/templateManage/queryTemplateByCondition",
            type : 'post',
            dataType:'text',
            data:{
                ...
                if(result.resultCode == "0") {
                    ...
                    if(list.length == 0){
                        ...
                        for ( var i = 0; i < list.length; i++) {
                            ...
                            html += "<span class='coupon_submit' ><a target='view_window' style='color:#FFFFFF' href='"+path+"/templateManage/showEditTemplate?pageNum="+page.pageNum+"&templatename="+TemplateBean.template_name+"' >编辑</a></span>";
                            if(TemplateBean.account_number=='system'){
                            	 html += " <span class='coupon_pass gray'><a style='color:#FFFFFF' href='javascript:;' >删除</a></span>";
                            }else{
                            ...
                            if(TemplateBean.template_name=='left'){
                                 html += " <span class='coupon_pass gray'><a target='view_window' style='color:#FFFFFF' >详情</a></span>";
                            }else{
                            	 html += " <span class='coupon_chakan'><a target='view_window' style='color:#FFFFFF' href='"+path+"/templateManage/recreateTemplate?fileName="+TemplateBean.filename+"' >详情</a></span>";
                            }
                            html += "</td>";
                            html += "</tr>";
  ...
  //显示注销模板确认弹窗
	...
	confirmSubmit : function(){
		...
		$.ajax({
			url:path+"/templateManage/SubmitTemplate",
			type : 'post',
			dataType:'text',
			data:{
	...
	countBean: function(){
		...
		$.ajax({
			url:path+"/templateManage/CountBean",
			type : 'post',
			dataType:'text',
			data:{
				"templateName" : templateName// 模板name
