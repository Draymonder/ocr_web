...
var UserManageJS = {
	...
	createUser : function(pageNum){
		window.location.href=path+"/userManage/showCreateUser?pageNum="+pageNum;
	},
	...
	queryUserByCondition : function(pageNum){
		var condition = $.trim($("#userName").val());
		//校验查询条件是否为空
		if(condition=='请输入用户名或手机号'){
			window.location.href = path + "/userManage/showUserManage?pageNum=1";
			return;
		}
		//密码空格校验
		...
		$.ajax({
			url:path+"/userManage/queryUserByCondition",
			type : 'post',
			dataType:'text',
			data:{
			...
			success:function(result){
				...
				if(result.resultCode == "0"){
					...
					head += "<td class='coupon_table_title'>用户名</td>";
					...
					var html = "";
						...
						if(list.length == 0){
							...
							html += "<td class='coupon_table_common'>"+userBean.fAccountNumber+"</td>";
							...
							if(userBean.fUserLevel != '2'){
								html += "<a href='javascript:;' class='coupon_pass' onclick=UserManageJS.showRemoveUser('"+userBean.fAccountNumber+"')>注销</a>";
							}else{
