...
var UserManageJS = {
		...
		goToPage : function(pageCount){
			...
			window.location.href = path + "/factoryManage/showfactoryManage?pageNum="+pageNum;
		},
	...
	createUser : function(pageNum){
		window.location.href=path+"/factoryManage/showCreatefactory?pageNum="+pageNum;
	},
	...
	confirmRemoveUser : function(){
		//alert(userAccount);
		$.ajax({
			url:path+"/factoryManage/removefactory",
			type : 'post',
			dataType:'text',
			data:{
				"id" : userAccount // 权限ID
			...
			success:function(result){
				...
				if (result.resultCode == "0") {
					window.location.href = path + "/factoryManage/showfactoryManage?pageNum=1";
				} else {
	...
	queryUserByCondition : function(pageNum){
		...
		$.ajax({
			url:path+"/factoryManage/queryfactoryByCondition",
			type : 'post',
			dataType:'text',
			data:{
			...
			success:function(result){
				...
				if(result.resultCode == "0"){
					...
					head += "<td class='coupon_table_title'>工厂名称</td>";
					head += "<td class='coupon_table_title'>工厂编号</td>";
					head += "<td class='coupon_table_title'>工厂地址</td>";
					...
					var html = "";
						...
						if(list.length == 0){
							...
							html += "<a href='"+path+"/factoryManage/showEditfactory?id="+userBean.id+"&pageNum="+page.pageNum+"' class='coupon_submit' >编辑</a>";
							//html += "<a href='"+path+"/factoryManage/showPermission?id="+userBean.id+"&pageNum="+page.pageNum+"' class='coupon_submit'>权限管理</a>";
							//if(userBean.fUserLevel != '0'){
