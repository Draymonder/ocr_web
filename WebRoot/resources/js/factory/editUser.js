...
var EditUserJS = {
	...
	editUser : function(pageNum){
		...
		var id = $("#factoryId").html();
		var name = $("#factoryName").val();
		var number = $("#factoryNumber").val();
		var address = $("#factoryAddress").val();
	...
	//	if(userPassword=="如不填,密码不修改"){
	 ...
	 // if(roleNumber=="" || roleNumber==null){
	  ...
	  //}
		if(name=="" ||name==null){
			showDialog1("工厂名称不能为空");
			return;
		}
		if(number=="" || number==null){
			showDialog1("工厂编号不能为空");
			return;
		}
		if(address=="" || address==null){
			showDialog1("工厂地址不能为空");
			return;
		}
...
//	}
		...
		$.ajax({
			url:path+"/factoryManage/editfactory",
			type : 'post',
			dataType:'text',
			data:{
