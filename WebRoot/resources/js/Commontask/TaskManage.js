...
var TaskManageJS = {
    ...
    goToPage : function(pageCount){
        ...
        window.location.href = path + "/CommonRecognizeManage/recognitionTask?pageNum="+pageNum;
    },
    ...
    queryTaskByCondition : function(pageNum){
        ...
        if(condition=='请输入工厂名称'&&begindate==''&&enddate=='') {
            window.location.href = path + "/CommonRecognizeManage/recognitionTask?pageNum=1";
            return;
        }

       
        $.ajax({
            url:path+"/CommonRecognizeManage/queryTaskListByCondition",
            type : 'post',
            dataType:'text',
            data:{
