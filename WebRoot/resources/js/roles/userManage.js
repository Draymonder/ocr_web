...
var UserManageJS = {
	...
	createUser : function(pageNum){
		window.location.href=path+"/RoleManage/showCreateRole?pageNum="+pageNum;
	},
	...
	confirmRemoveUser : function(){
		//alert(userAccount);
		$.ajax({
			url:path+"/RoleManage/removeRole",
			type : 'post',
			dataType:'text',
			data:{
				"roleID" : userAccount // 角色ID
			},
	...
	queryUserByCondition : function(pageNum){
		var condition = $.trim($("#userName").val());//角色名称或者编号
		//校验查询条件是否为空
		if(condition=='请输入角色编号或者角色名称'){
			window.location.href = path + "/RoleManage/showRoleManage?pageNum=1";
			return;
		}
		//密码空格校验
		...
		$.ajax({
			url:path+"/RoleManage/queryRoleByCondition",
			type : 'post',
			dataType:'text',
			data:{
			...
			success:function(result){
				...
				if(result.resultCode == "0"){
					...
					head += "<td class='coupon_table_title'>角色名称</td>";
					...
					var html = "";
						...
						if(list.length == 0){
							...
							html += "<a href='"+path+"/RoleManage/showEditRole?id="+userBean.id+"&pageNum="+page.pageNum+"' class='coupon_submit' >编辑</a>";
							html += "<a href='"+path+"/RoleManage/showPermission?id="+userBean.id+"&pageNum="+page.pageNum+"' class='coupon_submit'>权限管理</a>";
							if(userBean.fUserLevel != '2'&&userBean.fUserLevel != '1'&&userBean.fUserLevel != '3'){
