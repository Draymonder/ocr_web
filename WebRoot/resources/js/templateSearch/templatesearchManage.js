...
var templateSearchManageJS = {
		...
		goToPage : function(pageCount){
			...
			window.location.href = path + "/templateSearchManage/showTemplateSearchManage?pageNum="+pageNum;
		},
	...
	updateStatus: function(pageNum){
		...
		$.ajax({
			url:path+"/templateSearchManage/updateStatus",
			type : 'post',
			dataType:'text',
			data:{
				"ids" : ids,
    ...
    queryUserByCondition : function(pageNum){
		...
		if(templateName=='请输入模板名称'&&factoryName=='请输入工厂名称'&&fileNumber=='请输入文件名'&&userName=='请输入人员名称'&&date==''&&date1==''){
			window.location.href = path + "/templateSearchManage/showTemplateSearchManage?pageNum=1";
			return;
		}
		...
		$.ajax({
			url:path+"/templateSearchManage/queryByCondition",
			type : 'post',
			dataType:'text',
			data:{
				...
				if(result.resultCode == "0"){
					...
					var html = "";
						...
						if(list.length == 0){
							...
							html += " <td><span><a class='coupon_submit' target='_blank' href='"+path+"/templateSearchManage/showDetail?id="+DataBean.id+"&pageNum="+page.pageNum+"' target='_blank'>详情</a></span>" +
                                "<span><a class='coupon_download' href='"+path+"/uploads/tiff/"+DataBean.fileNumber+"' target='_blank'>导出文件</a></span></td>";

							//html += "<td class='coupon_table_common'>"+userBean.fState+"</td>";
							
							//html += "<td class='coupon_table_common'>";
							//html += "<a href='"+path+"/userManage/userDetail?fAccountNumber="+userBean.fAccountNumber+"&pageNum="+page.pageNum+"' class='coupon_chakan' >查看</a>";
							//html += "<a href='"+path+"/userManage/showEditUser?fAccountNumber="+userBean.fAccountNumber+"&pageNum="+page.pageNum+"' class='coupon_submit' >编辑</a>";
							//html += "<a href='"+path+"/userManage/showPermission?fAccountNumber="+userBean.fAccountNumber+"&pageNum="+page.pageNum+"' class='coupon_submit'>权限管理</a>";
							//if(userBean.fUserLevel != '0'){
							//	html += "<a href='javascript:;' class='coupon_pass' onclick=templateSearchManageJS.showRemoveUser('"+userBean.fAccountNumber+"')>注销</a>";
							//}else{
