package com.eqy.web.service.common_templateSearch.imp;
...
@Service("CommonTemplateSearchService")
public class CommonTemplateSearchServiceImp implements ICommonTemplateSearchService {
	...
	public void updateData(Map<String, Object> map) {
		tNgcDataCommonMapper.updateData(map);
	...
	public void showTemplateSearchManage(HttpServletRequest request, Model model,HttpSession session) {
         ...
         Integer count =tNgcDataCommonMapper.countBean(map);
         Pagenation page = new Pagenation(pageNum, size, count);
         map.put("start", page.getStartRow());
         map.put("size", page.getSize());
         try{
         List<TNgcDataCommon> list =tNgcDataCommonMapper.selectTaskListByPage(map);
         System.out.println("数量list:"+list.size());
         for(TNgcDataCommon TNgcData:list){
	...
	public Map<String, Object> executeQueryByCondition(HttpServletRequest request) {
    	...
    	Map<String, Object> map = new HashMap<String, Object>();
    	List<TNgcDataCommon> list1=null;
    	int sl=0;
    	try {
    		//returnMap = userManageService.executeSelectUserByCondition(request);
                ...
                Integer count = this.tNgcDataCommonMapper.countBean(map);

                Integer size = DictionaryMethod.size;
                Pagenation page = new Pagenation(pageNum, size, count);
                map.put("start", page.getStartRow());
                map.put("size", page.getSize());
                list1 = this.tNgcDataCommonMapper.selectTaskListByPage(map);
                for(TNgcDataCommon TNgcData:list1){
               	 if(TNgcData.getAccountNumber()!=null){
               		  //通过账号查询到用户名,工厂名
               		 System.out.println("用户名"+TNgcData.getAccountNumber());
               		 String factoryID=UserBeanMapper.selectUserByAccountNum(TNgcData.getAccountNumber()).getFactory();
	...
	public void dispalyDetail(HttpServletRequest request, Model model) {
		...
		TNgcDataCommon data=new TNgcDataCommon();
        List<ShowDateBean> showList=new ArrayList<ShowDateBean>();
        try
        {   data=tNgcDataCommonMapper.selectByPrimaryKey(Integer.parseInt(id));
            map=MapUtil.sortMapByKey(tbCommonMapMapper.findAll(commonTemplateBeanMapper.selectBYTemplateName(data.getTemplateName())));//按key值排序   按通用模板名称找到通用模板id
            for (Map.Entry<String, String> entry : map.entrySet()) {
            	map1.put("id", id);
                map1.put("fieldName",entry.getKey());
             //   System.out.println(entry.getKey() + " " + tNgcDataCommonMapper.selectColumnByA(map1));

                if(tNgcDataCommonMapper.selectColumnByA(map1)==null||tNgcDataCommonMapper.selectColumnByA(map1).equals("")){

                }else{
                ShowDateBean bean=new ShowDateBean();
                bean.setFieldName(entry.getValue());
                bean.setColumns(entry.getKey());
                bean.setFieldVal(tNgcDataCommonMapper.selectColumnByA(map1));

                if(TbCommonDataMarkMapper.findDescription(map1)!=null){

                	bean.setDescription(TbCommonDataMarkMapper.findDescription(map1));
                    bean.setFlag(TbCommonDataMarkMapper.findflag(map1));
                }else{
                	bean.setDescription("");
                    bean.setFlag(null);
                }
	...
	public Map<String, Object> executeEditDetail(HttpServletRequest request) {
        ...
        try{
           for(String str:Arry){
        	...
        	map.put("id",id);
        	tNgcDataCommonMapper.updateDataByID(map);
        	map.put("flag1",SystemConstants.Data_SUCCESS);
        	tNgcDataCommonMapper.updateflag1(map);
        	resultCode="0";
           }
        }catch (Exception e)
        {
